...
describe("products", () => {
  it('should be able to search products', async () => {
    ...
    let listProducts = products.slice(_offset, _limit + _offset).map(item => {
      return item;
    })
...
describe("categories", () => {
  it('should be able to bulk delete categories', async () => {
    const { body, status } = await request(app).post(`/v1/categories/delete`).send({
...
describe("sellers", () => {
  it('should be able to blacklist a seller', async () => {
    const sellerId = 2;
    const { body, status } = await request(app).patch(`/v1/sellers/${sellerId}/blacklist`).send({
      blacklisted: true
    });
  ...
  it('should be able to search sellers', async () => {
    const { body, status } = await request(app).get(`/v1/sellers`).send({
      seller: "Samsung"
    });
    expect(status).toBe(httpStatus.OK);
    expect(body).toEqual({});
