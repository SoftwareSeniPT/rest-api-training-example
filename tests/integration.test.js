...
describe("products", () => {
  it('should be able to search products', async () => {
    const page = 1;
    const limit = 10;
    const { body, status } = await request(app).get(`${ver}/products?page=${page}&limit=${limit}`);
  ...
  it('should be able to get product details', async () => {
    const { body, status } = await request(app).get(`${ver}/products/0`);
    expect(status).toBe(httpStatus.OK);
    expect(body).toEqual(products[0]);
  });

  it('should be able to update product name', async () => {
    const { body, status } = await request(app).patch(`${ver}/products/0`).send({ name: "Samsung S10" });
    expect(status).toBe(httpStatus.OK);
    expect(body).toEqual(products[0]);
  });

  it('should be returning error when creating a product', async () => {
    const { body, status } = await request(app).post(`${ver}/products`).send({
      name: "Samsung S10",
  ...
  it('should be returning error deleting a product', async () => {
    const { body, status } = await request(app).delete(`${ver}/products/0`);
    expect(status).toBe(httpStatus.INTERNAL_SERVER_ERROR);
    expect(body).toEqual({
      error: "PRODUCT_ERROR",
...
describe("categories", () => {
  it('should be able to bulk delete categories', async () => {
    const { body, status } = await request(app).delete(`${ver}/categories`).send({
...
describe("sellers", () => {
  it('should be able to blacklist a seller', async () => {
    const { body, status } = await request(app).patch(`${ver}/sellers/2`).send({
      blacklisted: true
    });
    expect(status).toBe(httpStatus.OK);
    expect(body).toEqual({});
  ...
  it('should be able to search sellers', async () => {
    const { body, status } = await request(app).get(`${ver}/sellers`).send({
      seller: "Samsung"
    });
    expect(status).toBe(httpStatus.OK);
    expect(body).toEqual({});
