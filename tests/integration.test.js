...
describe("products", () => {
  it('should be able to search products', async () => {
    ...
    const seller = 'Jaya Phone';
    const { body, status } = await request(app).get(`/v1/products?q=${name}&category=${category}&seller=${seller}&page=${page}&limit=${limit}`);
...
describe("sellers", () => {
  it('should be able to blacklist a seller', async () => {
    const { body, status } = await request(app).patch(`/v1/block-seller`).send({
      sellerId: 2,
      blacklisted: true
    });
