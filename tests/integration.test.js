...
describe("products", () => {
  it('should be able to search products', async () => {
    ...
    const { body, status } = await request(app).get(`/v1/products?name=${name}&category=${category}&seller=${seller}&page=${page}&limit=${limit}&sort=${sort}&sortBy=${sortBy}`);
  ...
  it('should be returning error deleting a product', async () => {
    const { body, status } = await request(app).delete(`/v1/products/0`).send();
    expect(status).toBe(httpStatus.INTERNAL_SERVER_ERROR);
    expect(body).toEqual({
      error: "PRODUCT_ERROR",
...
describe("sellers", () => {
  it('should be able to blacklist a seller', async () => {
    const sellerId = 2;
    const { body, status } = await request(app).put(`/v1/sellers/block/${sellerId}`).send({
      blacklisted: true
    });
    expect(status).toBe(httpStatus.OK);
    expect(body).toEqual({});
