...
describe("categories", () => {
  it('should be able to bulk delete categories', async () => {
    const { body, status } = await request(app).delete(`/categories/delete`).send({
...
describe("sellers", () => {
  it('should be able to blacklist a seller', async () => {
    const { body, status } = await request(app).patch(`/seller-block`).send({
      sellerId: 2,
      blacklisted: true
    });
  ...
  it('should be able to search sellers', async () => {
    ...
    const { body, status } = await request(app).get(`/sellers?seller=${seller}`);
    expect(status).toBe(httpStatus.OK);
    expect(body).toEqual({});
  });
});