...
describe("products", () => {
  it('should be able to search products', async () => {
    const page = 1;
    const limit = 10;
    const { body, status } = await request(app).get(`/v1/products?q=Samsung&category=Phone&seller=Samsung&page=${page}&limit=${limit}`);
...
describe("sellers", async () => {
  it('should be able to blacklist a seller', async () => {
    const { body, status } = await request(app).patch(`/v1/sellers`).send({
      sellerId: 2,
      blacklisted: true
    });
    expect(status).toBe(httpStatus.OK);
    expect(body).toEqual(sellers[2]);
  });
