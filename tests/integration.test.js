...
describe("products", () => {
  it('should be able to search products', async () => {
    const page = 1;
    const limit = 1;
    const { body, status } = await request(app).get(`/api/v1/products?product=Samsung&seller=Samsung&category=Phone&page=${page}&limit=${limit}`);
  ...
  it('should be returning error deleting a product', async () => {
    const { body, status } = await request(app).delete(`/api/v1/products/0`);
    expect(status).toBe(httpStatus.INTERNAL_SERVER_ERROR);
    expect(body).toEqual({
      error: "PRODUCT_ERROR",
...
describe("categories", () => {
  it('should be able to bulk delete categories', async () => {
    const { body, status } = await request(app).post(`/api/v1/categories`).send({
...
describe("sellers", () => {
  it('should be able to blacklist a seller', async () => {
    const { body, status } = await request(app).patch(`/api/v1/sellers/2`).send({
      blacklisted: true
    });
    expect(status).toBe(httpStatus.OK);
    expect(body).toEqual({});
  ...
  it('should be able to search sellers', async () => {
    const page = 1;
    const limit = 10;
    const { body, status } = await request(app).get(`/api/v1/sellers?product=Samsung&seller=Samsung&page=${page}&limit=${limit}`);
    expect(status).toBe(httpStatus.OK);
    expect(body).toEqual({
      items: [],
      pagination: {
