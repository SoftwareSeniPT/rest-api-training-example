...
describe("products", () => {
  it('should be able to search products', async () => {
    ...
    const { body, status } = await request(app).get(`${version}/products?category=${category}&seller=${seller}&page=${page}&limit=${limit}`);
  ...
  it('should be able to get product details', async () => {
    const { body, status } = await request(app).get(`${version}/products/0`);
    expect(status).toBe(httpStatus.OK);
    expect(body).toEqual(products[0]);
  });

  it('should be able to update product name', async () => {
    const { body, status } = await request(app).patch(`${version}/products/0`).send({ name: "Samsung S10" });
    expect(status).toBe(httpStatus.OK);
    expect(body).toEqual(products[0]);
  });

  it('should be returning error when creating a product', async () => {
    const { body, status } = await request(app).post(`${version}/products`).send({
      name: "Samsung S10",
  ...
  it('should be returning error deleting a product', async () => {
    const { body, status } = await request(app).delete(`${version}/products/0`);
    expect(status).toBe(httpStatus.INTERNAL_SERVER_ERROR);
    expect(body).toEqual({
      error: "PRODUCT_ERROR",
...
describe("categories", () => {
  it('should be able to bulk delete categories', async () => {
    const { body, status } = await request(app).delete(`${version}/categories`).send({
...
describe("sellers", () => {
  it('should be able to blacklist a seller', async () => {
    const sellerId = 2;
    const { body, status } = await request(app).patch(`${version}/sellers/${sellerId}`).send({
      blacklisted: true
    });
    expect(status).toBe(httpStatus.OK);
    expect(body).toEqual({});
  ...
  it('should be able to search sellers', async () => {
    const { body, status } = await request(app).get(`${version}/sellers`).send({
      seller: "Samsung"
    });
    expect(status).toBe(httpStatus.OK);
    expect(body).toEqual({});
